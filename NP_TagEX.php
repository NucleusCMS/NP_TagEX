...
public $cuURL;
    ...
    function getName()              {return 'Tags Extension';}
    function getAuthor()            {return 'nakahara21 + shizuki + Tucker + Cacher';}
...
// Escape SQL query strings
        if (is_array($value)) {
            if (get_magic_quotes_gpc()) $value = array_map("stripslashes", $value);
            if (!array_map("is_numeric",$value)) $value = array_map("sql_real_escape_string", $value);
            else                                 $value = intval($value);
        } else {
            if (get_magic_quotes_gpc()) $value = stripslashes($value);
            if (!is_numeric($value))    $value = "'" . sql_real_escape_string($value) . "'";
            else                        $value = intval($value);
        }
        return $value;
    }
 ...
 */
    ...
    function install()
    {
        $tplAND = '<span class="tagex_and_or"> <a href="<%andurl%>" title="narrow">&amp;</a>.';
        $tplOR  = '<a href="<%orurl%>" title="expand">or</a> </span>';
        $tplIDX = <<<__ORTAGTPL__
<%and%><%or%>
